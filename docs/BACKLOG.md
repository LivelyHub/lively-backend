...
- [x] Response matches CORE.md §7 shape: `period`, `range_start`, `range_end`, `has_data`, `headline`, `consistency_pct`, `exercise_completion_pct`, `medication_adherence_pct`, `chair_test_latest`, `chair_test_delta`, `highlights[]`, `areas_needing_support[]`
- [x] `consistency_pct` = % of days in range with ≥1 engagement row — `month` is a rolling 30-day window, not a calendar month
- [x] `chair_test_latest`/`chair_test_delta` are numeric (latest reps, and change from first to last in range), not a categorical trend string — `chair_test_delta` is `null` with fewer than 2 chair tests in range
- [x] Copy tone: `headline`/`highlights` always lead positive (highlights are opt-in per category, never spun from a bad number); `areas_needing_support` is encouragement-framed, never guilt
- [x] Zero-data elder → `has_data: false`, `consistency_pct: 0`, empty arrays, `chair_test_latest`/`delta: null`, still 200 with a real (not empty) gentle headline mobile renders directly in its empty state

**Rewritten mid-build, and worth knowing why:** the first version of this endpoint used `range: {from,to}`, `chair_test_trend: 'improving'|'stable'|'declining'`, and English copy — all matching CORE.md §7's own literal example at the time. During the mobile↔backend merge (2026-07-17) it turned out `lively-mobile`'s actual report screen (`app/report.tsx`) and its reference algorithm (`lib/api/mocks/computeReport.ts`) were built against a different, more specific shape: flat `range_start`/`range_end`, a `has_data` flag, numeric `chair_test_latest`/`chair_test_delta` (the UI renders an actual "+3" delta chip, not just a direction), and — the thing actually worth flagging — Indonesian copy, matching the rest of the product. The English copy was never a real product decision, just an artifact of CORE.md's own example being wrong. Backend now mirrors `computeReport.ts` exactly (same algorithm, same copy), including its more correct medication-adherence math: a medication only counts as "scheduled" on days on or after its `created_at`, and a day's `taken` count is capped at that day's `scheduled` count so a duplicate log can't inflate the percentage.

**Test:** verified against the real seeded Eyang Uti data on Neon, every field checked by hand against the mobile team's exact algorithm — week: `consistency_pct = round(5/7×100) = 71` (union of chair/exercise/medication activity days), `exercise_completion_pct = round(4/7×100) = 57`, `chair_test_latest: 12` with `chair_test_delta: null` (only 1 of 4 seeded chair tests falls in a 7-day window), highlights/areas both matched the exact conditional branches for those numbers. Month view pulls in all 4 chair tests (`8→12`, `delta: 4`) with different `consistency_pct`/`range`. Fresh elder → `has_data: false` with the real Indonesian zero-state headline, not an error. Invalid `period` → 400. Cross-family → 404.
**Depends on:** B5.3.
